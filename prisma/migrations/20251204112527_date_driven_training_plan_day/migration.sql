...
-- AlterTable: Update TrainingPlan field names (only if columns exist)
DO $$ 
BEGIN
    -- Rename columns if they exist
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'TrainingPlan' AND column_name = 'trainingPlanName') THEN
        ALTER TABLE "TrainingPlan" RENAME COLUMN "trainingPlanName" TO "name";
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'TrainingPlan' AND column_name = 'trainingPlanGoalTime') THEN
        ALTER TABLE "TrainingPlan" RENAME COLUMN "trainingPlanGoalTime" TO "goalTime";
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'TrainingPlan' AND column_name = 'goalFiveKPace') THEN
        ALTER TABLE "TrainingPlan" RENAME COLUMN "goalFiveKPace" TO "goalPace5K";
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'TrainingPlan' AND column_name = 'trainingPlanStartDate') THEN
        ALTER TABLE "TrainingPlan" RENAME COLUMN "trainingPlanStartDate" TO "startDate";
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'TrainingPlan' AND column_name = 'trainingPlanTotalWeeks') THEN
        ALTER TABLE "TrainingPlan" RENAME COLUMN "trainingPlanTotalWeeks" TO "totalWeeks";
    END IF;
END $$;
