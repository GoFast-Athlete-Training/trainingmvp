...
export async function GET(request: NextRequest) {
  ...
  const rows = await prisma.taper_config.findMany({ orderBy: { updatedAt: "desc" }, include });
  return NextResponse.json({ success: true, tapers: rows });
}
...
export async function POST(request: NextRequest) {
  ...
  const row = await prisma.taper_config.create({
    data: { name: body.name?.trim() || "New taper" },
    include,
  });
